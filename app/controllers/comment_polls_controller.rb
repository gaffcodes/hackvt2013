class CommentPollsController < PollsController
  before_filter :authorize_legislator!, :except => [:index, :show]
  before_action :set_comment_poll, only: [:show, :edit, :update, :destroy, :vote_agree, :vote_oppose]
...
def vote_agree
	@comment_poll.votes = @comment_poll.votes + 1
	@comment_poll.in_favor = @comment_poll.in_favor + 1
	@comment_poll.save
end
